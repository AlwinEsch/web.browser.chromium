...
{
#if defined(HAS_GL) || defined(HAS_GLES)
  m_renderer = new CWorkRendererGL(m_client);
#elif defined(HAS_DX)
  m_renderer = new CWorkRendererDirectX(m_client);
#else
...
bool CRendererClient::Initialize(int width, int height)
{
...
{
  if (m_client && !m_client->ContextMenuOpen() && kodi::GetSettingBoolean("main.editdialogdirect"))
  {
    std::thread([this, input_mode] {
