...
/*
TODO:
- Make more user language related (currently mostly english)
- Maybe a nicer webview?
*/
...
std::string CURICheck::GetContentStatusString(cef_ssl_content_status_t status)
{
  std::string result;
...
std::string CURICheck::GetTimeString(const CefTime& value)
{
  if (value.GetTimeT() == 0)
    return "Unspecified";
...
std::string CURICheck::GetBinaryString(CefRefPtr<CefBinaryValue> value)
{
  if (!value.get())
    return "&nbsp;";
