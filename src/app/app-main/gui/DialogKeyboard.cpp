...
#include "../../../../lib/kodi-dev-kit/include/kodi/gui/controls/Button.h"
...
/*
TODO:
- About keyboard need checked if works with characters on all OS
- To have also a left/right swapping keyboard for all cases
- Easy user GUI for website still need a lot todo and this here maybe long away from final?
*/
...
void CBrowserDialogKeyboard::Show(CefRefPtr<CWebBrowserClient> client,
...
bool CBrowserDialogKeyboard::OnAction(const kodi::gui::input::CAction& action)
{
  ...
  if (m_client && action.GetButtonCode() & KEY_VKEY)
  {
    ...
    key_event.windows_key_code =
        CSystemTranslator::ButtonCodeToKeyboardCode(action.GetButtonCode());
    key_event.native_key_code = 0;
    key_event.is_system_key = false;
    key_event.character = action.GetUnicode();
    key_event.unmodified_character =
        CSystemTranslator::ButtonCodeToUnmodifiedCharacter(action.GetButtonCode());
