...
process_opts() {
  ...
  while getopts ":b:g:lm:scht:d:un" OPTNAME; do
    case "$OPTNAME" in
      b)
...
if [ -z $distname ]; then
  ...
  printf "The following target is installed in a chroot: %s\n" $distname
fi
echo
...
if [ "$(uname -m)" = x86_64 ]; then
  if [ -z $arch ]; then
    ...
    printf "You are running a 64bit kernel and %ibit selected from options\n" "${arch}"
  fi
