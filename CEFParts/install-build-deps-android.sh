...
#  python ${BASE_PATH}/chromium/src/build/get_sdk_extras_packages.py
#  cd ${BASE_PATH}

# Note: Changed from java-1.7.0-openjdk-amd64 to java-1.7.0-openjdk-i386, OK?
if ! sudo update-java-alternatives -s java-1.7.0-openjdk-i386 \
...
if test "$skip_inst_sdk_packages" != 1; then
  "$(dirname "${BASH_SOURCE[0]}")/chromium/src/build/install-android-sdks.sh"
fi
